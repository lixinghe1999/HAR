...
if __name__ == '__main__':
    ...
    if args.prepare:
        ...
        for i in tqdm(range(1, 10)):
            ...
            Xs.append(np.load(dataset_folder + x_name))
            Ys.append(np.load(dataset_folder + y_name))
        ...
        # Y_count['met'] = met
        # save as csv
        Y_count.to_csv('resources/Y_count.csv')
        ...
        plt.savefig('figs/capture24_label_distribution.pdf')
        Y_map = {x: i if x in topk_labels else 'other' for i, x in enumerate(Y_count.index)}
        
        anno_label_dict = pd.read_csv(os.path.join(dataset_folder, 'annotation-label-dictionary.csv'), 
