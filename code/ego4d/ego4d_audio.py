...
def discrimate_train(train_dataset, test_dataset, model, optimizer, device, num_epochs=10):
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(pbar):
            ...
            imu_output = model(data['audio'].to(device))
...
if __name__ == "__main__":
    ...
    # Define the model
    train_dataset = Ego4D_Moment(window_sec=2.5, modality=['audio', 'cluster_label'], split='train')
    ...
    audio_model = AudioTagging()
    audio_model = Head(audio_model, size_embeddings=3840, n_classes=train_dataset.num_class)
